    ...
    if(isset($_POST['add_patient']))
{
    ...
    $pat_email = $_POST['pat_email']; // Capture email
    ...
    $pat_pwd = password_hash($_POST['pat_pwd'], PASSWORD_DEFAULT); // Hash password before storing

    // Update SQL query to include email and password
    $query = "INSERT INTO his_patients (pat_fname, pat_lname, pat_email, pat_ailment, pat_age, pat_dob, pat_number, pat_phone, pat_type, pat_addr, pat_pwd) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param('sssssssssss', $pat_fname, $pat_lname, $pat_email, $pat_ailment, $pat_age, $pat_dob, $pat_number, $pat_phone, $pat_type, $pat_addr, $pat_pwd);
    $stmt->execute();
...
<html lang="en">
    ...
    <input type="email" required="required" name="pat_email" class="form-control" id="pat_email" placeholder="Patient's Email">
</div>

<div class="form-group col-md-6">
    <label for="pat_pwd" class="col-form-label">Password</label>
    <input type="password" required="required" name="pat_pwd" class="form-control" id="pat_pwd" placeholder="Enter Password">
</div>
